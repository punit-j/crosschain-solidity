...
contract('Bridge - [admin]', async accounts => {
    ...
    beforeEach(async () => {
        BridgeInstance = await BridgeContract.new(chainID, 0, backendSrv);
    });
    ...
    it('Bridge admin should be expectedBridgeAdmin', async () => {
        assert.equal(await BridgeInstance._ownerAddress(), expectedBridgeAdmin);
    });

    it('Bridge admin should be changed to expectedBridgeAdmin', async () => {
        const expectedBridgeAdmin2 = accounts[1];
        TruffleAssert.passes(await BridgeInstance.adminChangeOwner(expectedBridgeAdmin2))
        assert.equal(await BridgeInstance._ownerAddress(), expectedBridgeAdmin2);
    });
    ...
    it('Should withdraw funds', async () => {
        ...
        await ERC20MintableInstance.mint(ERC20HandlerInstance.address, numTokens);
